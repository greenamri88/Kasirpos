...
<body>
    ...
    const app = {
        ...
        /* ==========================================================================
           9. LAPORAN & LABA RUGI (EXPORT EXCEL & PDF WITH DATE FILTER)
   ========================================================================== */
async renderReports(container) {
    ...
    // Default filter: 'month' (Bulan Ini) jika belum pernah diset
    if (!this.reportFilterType) this.reportFilterType = 'month';

    // Helper untuk memformat objek Date ke YYYY-MM-DD
    const formatDate = (date) => date.toISOString().split('T')[0];

    // Tentukan Tanggal Mulai (startDate) & Tanggal Selesai (endDate)
    const today = new Date();
    let startDate = '';
    let endDate = formatDate(today);

    if (this.reportFilterType === 'today') {
        startDate = formatDate(today);
    } else if (this.reportFilterType === 'week') {
    ...
    } else if (this.reportFilterType === 'custom') {
        startDate = this.reportStartDate || formatDate(today);
        endDate = this.reportEndDate || formatDate(today);
    }

    // 2. Filter data Penjualan dan Pengeluaran berdasarkan tanggal
    ...
    // 3. Kalkulasi Angka Laba Rugi
    const totalOmzet = filteredSales.reduce((a, c) => a + Number(c.total || 0), 0);
    const totalHpp = filteredSales.reduce((a, c) => a + Number(c.totalHpp || 0), 0);
    const totalExp = filteredExpenses.reduce((a, c) => a + Number(c.amount || 0), 0);
    const grossProfit = totalOmzet - totalHpp;
    const netProfit = grossProfit - totalExp;

    // Simpan data terfilter agar bisa digunakan oleh fungsi export Excel/PDF
    this.currentFilteredSales = filteredSales;
    this.currentFilteredExpenses = filteredExpenses;
    ...
    container.innerHTML = `
        ...
        <div class="card" style="margin-bottom: 1rem;">
            ...
            <div style="display:flex; gap:8px; flex-wrap:wrap; margin-bottom:12px;">
                <button class="btn ${this.reportFilterType === 'today' ? 'btn-primary' : 'btn-secondary'}" onclick="app.setReportFilter('today')">Hari Ini</button>
                <button class="btn ${this.reportFilterType === 'week' ? 'btn-primary' : 'btn-secondary'}" onclick="app.setReportFilter('week')">7 Hari Terakhir</button>
                <button class="btn ${this.reportFilterType === 'month' ? 'btn-primary' : 'btn-secondary'}" onclick="app.setReportFilter('month')">Bulan Ini</button>
                <button class="btn ${this.reportFilterType === 'year' ? 'btn-primary' : 'btn-secondary'}" onclick="app.setReportFilter('year')">Tahun Ini</button>
                <button class="btn ${this.reportFilterType === 'custom' ? 'btn-primary' : 'btn-secondary'}" onclick="app.setReportFilter('custom')">Custom Rentang</button>
            </div>

            ${this.reportFilterType === 'custom' ? `
                <div style="display:flex; gap:10px; align-items:center; background:var(--bg-subtle, #f5f5f5); padding:10px; border-radius:6px;">
                    <div>
                        <label style="font-size:0.75rem; display:block;">Dari Tanggal:</label>
                        <input type="date" id="repStartDate" value="${startDate}" onchange="app.handleCustomDateChange()" style="padding:4px 8px;">
                    </div>
                    <div>
                        <label style="font-size:0.75rem; display:block;">Sampai Tanggal:</label>
                        <input type="date" id="repEndDate" value="${endDate}" onchange="app.handleCustomDateChange()" style="padding:4px 8px;">
                    </div>
                </div>
            ` : ''}
        </div>

        <!-- CARD RINGKASAN LABA RUGI -->
        <div class="card">
            <div class="card-title">Laporan Laba Rugi</div>
            <div style="font-size:0.8rem; color:#666; margin-bottom:10px;">
                Periode: <b>${startDate}</b> s/d <b>${endDate}</b>
            </div>
            <div style="font-size:0.85rem;">
                <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid var(--border);">
                    <span>Total Penjualan (Omzet)</span>
                    <b>Rp ${totalOmzet.toLocaleString('id-ID')}</b>
                </div>
                <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid var(--border); color:var(--danger);">
                    <span>Total HPP (Beban Pokok)</span>
                ...
                <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid var(--border); font-weight:bold;">
                    ...
                    <span style="color:var(--success);">Rp ${grossProfit.toLocaleString('id-ID')}</span>
                </div>
                <div style="display:flex; justify-content:space-between; padding:6px 0; border-bottom:1px solid var(--border); color:var(--danger);">
                    <span>Total Beban Operasional</span>
                    <span>- Rp ${totalExp.toLocaleString('id-ID')}</span>
                </div>
                <div style="display:flex; justify-content:space-between; padding:10px 0; font-weight:800; font-size:1.05rem;">
                    <span>LABA BERSIH:</span>
                    <span style="color:${netProfit >= 0 ? 'var(--success)' : 'var(--danger)'};">Rp ${netProfit.toLocaleString('id-ID')}</span>
                </div>
            </div>
        </div>
        ...
        <div class="card" style="margin-top:1rem;">
            <div class="card-title">Export Laporan Terfilter</div>
            <div class="grid-2">
...
// --- HANDLER FUNGSI FILTER ---
setReportFilter(type) {
    this.reportFilterType = type;
    const container = document.getElementById('reportContainer') || document.querySelector('.main-content');
    this.renderReports(container);
},

handleCustomDateChange() {
    this.reportStartDate = document.getElementById('repStartDate').value;
    this.reportEndDate = document.getElementById('repEndDate').value;
    const container = document.getElementById('reportContainer') || document.querySelector('.main-content');
    this.renderReports(container);
},
...
async exportExcel() {
    const sales = this.currentFilteredSales || [];
    const expenses = this.currentFilteredExpenses || [];
    ...
    const salesData = sales.map(s => ({
        ...
        Total: s.total,
        HPP: s.totalHpp,
        Laba_Kotor: (s.total || 0) - (s.totalHpp || 0)
    }));
    ...
    const p = this.currentReportPeriod;
    XLSX.writeFile(wb, `Laporan_POS_${p ? p.startDate + '_sdr_' + p.endDate : 'terfilter'}.xlsx`);
},
...
async exportPDF() {
    ...
    doc.setFontSize(16);
    doc.text(this.settings?.storeName || 'Laporan Kasir', 14, 15);
    doc.setFontSize(10);
    doc.text(`Laporan Keuangan & Penjualan (${p.startDate} s/d ${p.endDate})`, 14, 22);

    const sales = this.currentFilteredSales || [];
    const tableData = sales.map(s => [s.id, s.date, s.platform, s.customer, `Rp ${(s.total || 0).toLocaleString('id-ID')}`]);
    ...
    doc.save(`Laporan_POS_${p.startDate + '_sdr_' + p.endDate}.pdf`);
}
